<script lang="ts">
    ...
    import GalListLoadingPage from "$lib/components/GalListLoadingPage.svelte";
    ...
    let GALs = $state<any[]>([]);
    let filteredGALs = $state<any>([]);
    let paginatedGALs = $state<any>([]);
    ...
    $effect(() => {
        ...
        filteredGALs = GALs.filter(gal => {
            ...
            const matchesSearch = $SelectedSearchQuery ?
                gal.Denumire_GAL.toLowerCase().includes($SelectedSearchQuery.toLowerCase()) :
                true;
...
$effect(()=>{
    if (filteredGALs.length) {
        const start = (currentPage - 1) * itemsPerPage;
        paginatedGALs = filteredGALs.slice(start, start + itemsPerPage);
    }else{
        paginatedGALs=[];
    }
})
...
{#if loading}
    <GalListLoadingPage />
{:else if error}
    <p class="error">Error loading GALs: {error}</p>
{:else if GALs.length}
